...
#if ( __EP0_LOG__ > 0)	
	printk("\n[INFO][SETUP]: %x %x %x %x %x %x %x %x :%i\n", data_ptr[0], data_ptr[1], data_ptr[2], data_ptr[3], data_ptr[4],
	       data_ptr[5], data_ptr[6], data_ptr[7], priv->ep0_cur_ref);
#endif	
...
static void e967_usb_setup_isr(const struct device *dev)
{
...
#if ( __EP0_LOG__ > 0)	
	//printk("\n[INFO][SETUP ISR]: %x %x %x %x %x %x %x %x :%i\n", ptr[0], ptr[1], ptr[2], ptr[3], ptr[4],
	...
	if (err < 0) {
#if ( __EP0_LOG__ > 0)
		printk("[SETUP] put msg failed, err=%i\n", err);
#endif		
		goto _exit;
	}
...
void _e967_proc_ep0_d2h(const struct device *dev)
{
	...
	if (data_len == 0 || len == 0) {
		...
		net_buf_unref(nbuf);
#if (__EP0_LOG__ > 0)
		printk("[INFO] ep0_d2h : nbuf:0x%p done!\n", nbuf);
#endif		
	}
