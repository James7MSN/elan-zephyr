...
static int test_sha256_large_data(void)
{
    ...
    LOG_INF("***** SHA-256 Large Data Test (>300KB) *****");
    LOG_INF("Testing data sizes: 300KB, 512KB, 1MB");

    for (size_t si = 0; si < ARRAY_SIZE(test_sizes); ++si) {
        size_t len = test_sizes[si];
        uint8_t *buf = k_malloc(len);
        if (!buf) {
            LOG_ERR("Alloc failed for %zu bytes (%.1f MB)", len, (float)len / (1024*1024));
            return -ENOMEM;
        }

        LOG_INF("--- Testing %zu bytes (%.1f MB) ---", len, (float)len / (1024*1024));

        /* Fill deterministic pattern */
        for (size_t i = 0; i < len; ++i) {
            buf[i] = (uint8_t)((i * 7 + 13) & 0xFF);  /* pseudo-random pattern */
        }

        /* Reference: Process as single chunk */
...
{
    int ret;
    int total_test_suites = 6;  /* Capability, pattern, incremental, large consistency, boundaries, large data */
    int passed_suites = 0;
    int failed_suites = 0;
